...
// Create a service.
return_type ipc_service_create(const char *service_name, ipc_structure_type *ipc_structure, tag_type *tag)
{
    switch (system_call_service_create(service_name, &ipc_structure->input_mailbox_id, tag))
    {
    ...
    time_type timeout, tag_type *tag)
{
    ...
    do
    {
        ...
        // If timeout is 0 we never block. If it is MAX_TIME, we block as long as we like.
        if (timeout != 0)
        {
            system_sleep(1000);
        }
...
return_type ipc_service_connection_wait(ipc_structure_type *ipc_structure)
{
    ...
    while (TRUE)
    {
        ...
        switch (ipc_receive(ipc_structure->input_mailbox_id, &message_parameter, NULL))
        {
            ...
            // The message didn't fit into this mailbox. General IPC messages must fit into 1024 bytes, so it was probably some
            ...
            case STORM_RETURN_SUCCESS:
            {
                mailbox_id_type reply_mailbox_id = *(mailbox_id_type *) message_parameter.data;
                ...
                switch (message_parameter.message_class)
                {
                    ...
                    {
                        ipc_structure->output_mailbox_id = *(mailbox_id_type *) message_parameter.data;
                        {
                            memory_deallocate((void **) &buffer);
...
return_type ipc_service_connection_request(ipc_structure_type *ipc_structure)
{
    ...
    message_parameter.block = TRUE;
    *(mailbox_id_type *) message_parameter.data = ipc_structure->input_mailbox_id;

    switch (system_call_mailbox_send(ipc_structure->output_mailbox_id, &message_parameter))
    {
        case STORM_RETURN_MAILBOX_UNAVAILABLE:
        case STORM_RETURN_ACCESS_DENIED:
        {
    ...
    switch (system_call_mailbox_receive(ipc_structure->input_mailbox_id, &message_parameter))
    {
        case STORM_RETURN_MAILBOX_UNAVAILABLE:
        case STORM_RETURN_ACCESS_DENIED:
        {
...
return_type ipc_connection_establish(ipc_structure_type *ipc_structure)
{
    ...
    // Create an input mailbox which we will use to get data from this connection.
    // FIXME: Make the size dynamic.
    if (system_call_mailbox_create(&ipc_structure->input_mailbox_id, 1 * MB,
...
// Send an IPC message. The reason for having this wrapper is to make the implementation of shared memory regions totally
// transparent to the programs.
return_type ipc_send(mailbox_id_type mailbox_id, message_parameter_type *message_parameter)
{
    return_type return_value = system_call_mailbox_send(mailbox_id, message_parameter);
    ...
    switch (return_value)
    {
        ...
        // This should normally not happen. If it does, it means someone has added a return case in mailbox.c in the kernel,
        // without adding it here. Thus, if you get this, please fix this library and send us a patch.
        default:
        {
            return IPC_RETURN_FAILED_MAILBOX_SEND;
        }
...
// Receive an IPC message. The reason for having this is the same as for ipc_send.
return_type ipc_receive(mailbox_id_type mailbox_id, message_parameter_type *message_parameter, unsigned int *buffer_size)
{
    return_type return_value = system_call_mailbox_receive(mailbox_id, message_parameter);
    ...
    switch (return_value)
    {
        ...
        case STORM_RETURN_MAILBOX_MESSAGE_TOO_LARGE:
        {
            // This interpretation of the 'block' flag might not be entirely orthodox, but I choose to do it like this anyway...
            if (!message_parameter->block || buffer_size == NULL)
            {
                return IPC_RETURN_MESSAGE_TOO_LARGE;
            }

            // This might not be entirely beautiful, but I think it is a pretty elegant solution.
