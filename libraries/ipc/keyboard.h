// Abstract: Protocol used when communicating with keyboard servers.
...
// "Special keys" are keys that don't generate printable UTF-8  sequences (like F1, Escape etc). Note that in chaos, some keys
// that generate printable characters in other systems (for example Tab) are treated differently.
enum
{
...
// A keyboard packet is what gets sent whenever a key is pressed or released.
typedef struct
{
    // Does this packet contain a character code? If not, it is just one of the shift states that has been modified, or perhaps a
    // 'special key'.
    ...
    // If has_special_key is TRUE, this field contains the key code for this key.
    uint32_t special_key;

    // Eight bytes for the character code, encoded with UTF-8.
