...
// Initialise a connection to the PCI server matching the 'tag' identification. If multiple choices are available (and less than
// ten in figure), the first one is chosen.
// FIXME: This semantic is not really ideal. Either always use the first one or use none.
...
return_type pci_get_number_of_devices(ipc_structure_type *pci_structure, unsigned int *number_of_devices)
{
...
// Get the data for the devices of the given type installed in the system.
return_type pci_device_exists(ipc_structure_type *pci_structure, pci_device_probe_type *probe, pci_device_info_type **device_info,
                              unsigned int *number_of_devices)
{
    ...
    ipc_receive(pci_structure->input_mailbox_id, &message_parameter, &length);

    // message_parameter.data may have been reallocated, so store its new address.
...
return_type pci_allocate_buffer(void **physical_address, void **virtual_address, unsigned int size)
{
    if (system_call_memory_allocate(virtual_address, SIZE_IN_PAGES(size), FALSE) != MEMORY_RETURN_SUCCESS)
    {
