                             ...
                             unsigned int *characters)
{
    ...
    *number = 0;
    ...
    // If we're not interested in the number of characters slurped by this function, we might have set characters to NULL.
    if (characters != NULL)
    {
        *characters = index;
    }
                              ...
                              int size, int precision, int flags)
{
    ...
    // Okay, we have the string, but in reversed order. Lets do some more magic before we reverse it.
...
// Prints the given data to the output string, a la printf and friends.
...
// Print a string using the previously initialised va_list structure.
                            ...
                            va_list arguments)
{
    ...
    // Minimum number of digits for integers; maximum number of chars for string.
    int precision;
    ...
    while (format_string[input_index] != '\0')
    {
        ...
        {
            // Percent sign means we get a formatting code as the next character.
            ...
            case '%':
            {
                ...
                // Now, some formatting stuff can be given. Let's parse it.
                while (!done)
                {
                    switch (format_string[input_index])
                    {
