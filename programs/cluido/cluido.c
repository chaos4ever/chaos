// Abstract: cluido is the Command Line User Interface (CLUI) of the chaos operating system. It
// serves a somewhat similar function as the AmigaOS CLI, COMMAND.COM in DOS, CMD.EXE in OS/2 and
// Windows NT, or the Bourne Again SHell in GNU systems.
...
static void command_list_cleanup(void)
{
    ...
    while (node != NULL)
    {
        ...
        while (string[search_position] != '\0' && non_space_characters == 0)
        {
...
// Get the value for the given environment variable. Returns NULL if not found.
...
static int arguments_parse(char *source, arguments_type *destination)
{
    ...
    for (position = 0, argument = 0; source[position]; position++)
    {
        ...
        if (source[position] == '"')
        {
            ...
            while (source[position + length] != '"' && source[position + length] != '\0')
            {
                length++;
            }
...
// Update the currently edited text on screen. This one also sets the cursor correctly.
static void string_input_update(char *string, unsigned int cursor_position, bool string_shrinks)
{
    char temporary;

    console_print_formatted(&console_structure, "\e[u%s%s", string, string_shrinks ? " " : "");
...
static void string_input(unsigned int max_length, bool do_syntax_check, bool command_list, char *buffer)
{
             ...
             keyboard_packet.special_key == IPC_KEYBOARD_SPECIAL_KEY_ENTER))
    {
        ...
        if (event_type == CONSOLE_EVENT_KEYBOARD && keyboard_packet.key_pressed)
        {
            ...
            {
                if (keyboard_packet.right_control_down || keyboard_packet.left_control_down)
                {
                    switch (keyboard_packet.character_code[0])
                    {
                        ...
                        case 'B':
                        {
                            ...
                            {
                                input_string_position -= unicode_utf8_previous_character_length(string, input_string_position);
                                string_input_update(string, input_string_position, FALSE);
                            }
                            break;
                        }
                        ...
                        case 'F':
                        {
                            if (input_string_position < input_string_length)
                            {
                                input_string_position += unicode_utf8_next_character_length(string, input_string_position);
                                string_input_update(string, input_string_position, FALSE);
                            }
                            break;
                        }
                ...
                else if (input_string_length + string_length(keyboard_packet.character_code) < max_length)
                {
                    string_copy(temporary_string, &string[input_string_position]);
                    string_copy(&string[input_string_position], keyboard_packet.character_code);
                    input_string_position += string_length(keyboard_packet.character_code);
                    string_copy(&string[input_string_position], temporary_string);
                    input_string_length += string_length(keyboard_packet.character_code);
                    input_string_characters++;
            ...
            else
            {
                switch (keyboard_packet.special_key)
                {
                    ...
                    case IPC_KEYBOARD_SPECIAL_KEY_BACK_SPACE:
                    {
                        if (input_string_position > 0)
                        {
                            unsigned int character_length = unicode_utf8_previous_character_length(string, input_string_position);
                            unsigned int new_position = input_string_position - character_length;

                            string_copy(&string[new_position], &string[input_string_position]);
                    ...
                    case IPC_KEYBOARD_SPECIAL_KEY_TAB:
                    {
                        ...
                        if (do_syntax_check)
                        {
                            ...
                            {
                                if (string_compare_max(command[counter].name, string, input_string_position) == 0)
                                {
                    ...
                    case IPC_KEYBOARD_SPECIAL_KEY_NUMERIC_8:
                    {
                        ...
                        if (command_list && command_list_pointer->next != NULL)
                        {
                            for (index = 0; index < input_string_characters; index++)
                            {
                                temporary_string[index] = ' ';
                    ...
                    case IPC_KEYBOARD_SPECIAL_KEY_NUMERIC_2:
                    {
                        ...
                        if (command_list && command_list_pointer->previous != NULL)
                        {
                            for (index = 0; index < input_string_characters; index++)
                            {
                                temporary_string[index] = ' ';
    ...
    if (words > 0)
    {
        ...
        for (counter = 0; counter < number_of_commands; counter++)
        {
            if (!string_compare(first_word, command[counter].name))
            {
                ...
                command_pointer(parsed_command.number_of_arguments, parsed_command.argument_start);
