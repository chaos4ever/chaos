...
void command_nibbles(void)
{
    ...
    for (player_number = 0; player_number < MAX_PLAYERS; player_number++)
    {
        ...
        for (segment_number = 0; segment_number < MAX_WORM_LENGTH; segment_number++)
        {
    ...
    while (!done)
    {
        ...
        for (player_number = 0; player_number < MAX_PLAYERS; player_number++)
        {
            ...
            if (player[player_number].dead && player[player_number].active)
            {
...
static void restart_player(int player_number)
{
    ...
    for (counter = 0; counter < 1000 && restarted == FALSE; counter++)
    {
        ...
        if (playfield[x][y] == EMPTY)
        {
            ...
            for (segment_number = 0; segment_number < START_WORM_LENGTH; segment_number++)
            {
...
static void move_players(void)
{
    ...
    for (player_number = 0; player_number < MAX_PLAYERS; player_number++)
    {
        ...
        for (segment_number = MAX_WORM_LENGTH - 1; segment_number > 0; segment_number--)
        {
            player[player_number].body[segment_number].x_pos = player[player_number].body[segment_number - 1].x_pos;
            player[player_number].body[segment_number].y_pos = player[player_number].body[segment_number - 1].y_pos;
        }
...
// Read key delay exactly TIMEOUT milliseconds, and returns a key if pressed, and KEY_NOTHING if no key was pressed.
static char read_key(void)
{
#if FALSE
    ...
    do
    {
        ...
        if (key == KEY_NOTHING)
        {
            ...
            system_call_mailbox_receive(console_structure.input_mailbox_id, &message_parameter);
        }
