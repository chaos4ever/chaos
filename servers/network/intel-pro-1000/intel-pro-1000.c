...
static void e1000_init_addr(e1000_t *device, netdriver_address_type *network_address)
{
    ...
    // FIXME: This reads all zeroes for some odd reason?? If it wasn't so much
    // work, it would be interesting to boot up Minix 3 using the same virtualization
    // software and step through the debugger in this method...
    for (int i = 0; i < 3; i++) {
...
// static void e1000_init_buf(e1000_t *device)
// {
...
//     device->rx_desc_count = E1000_RXDESC_NR;
//     device->tx_desc_count = E1000_TXDESC_NR;
