    ...
    while (!done)
    {
        ...
        switch (message_parameter.message_class)
        {
            ...
            case IPC_ETHERNET_PACKET_SEND:
            {
                ...
                {
                    log_print(&log_structure, LOG_URGENCY_ERROR, "Failed to send an ethernet packet.");
            ...
            case IPC_ETHERNET_ADDRESS_GET:
            {
                ...
                system_call_mailbox_send(ipc_structure.output_mailbox_id, &message_parameter);
#endif
                break;
            }
...
int main(void)
{
    ...
    // Create the service.
    if (ipc_service_create("ethernet", &ipc_structure, &empty_tag) != IPC_RETURN_SUCCESS)
    {
        log_print(&log_structure, LOG_URGENCY_EMERGENCY, "Couldn't create ethernet service.");
        return -1;
    }
