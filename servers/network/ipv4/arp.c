...
// The ARP cache is a linked list with an entry for each host the  machine has been talking to recently.
...
bool arp_ip_to_ethernet_address(u32 ip_address, u8 ethernet_address[])
{
    ...
    while (entry != NULL)
    {
        ...
        {
            memory_copy(ethernet_address, entry->ethernet_address, IPV4_ETHERNET_ADDRESS_LENGTH);
...
void arp_insert_entry(u32 ip_address, u8 ethernet_address[])
{
    ...
    memory_copy(entry->ethernet_address, ethernet_address, IPV4_ETHERNET_ADDRESS_LENGTH);
    entry->time = time_get();
...
void arp_packet_receive(ipv4_interface_type *interface, ipv4_ethernet_header_type *ethernet_header,
                        int length __attribute__((unused)), mailbox_id_type output_mailbox_id)
{
    arp_packet_type *arp_packet = (arp_packet_type *) &ethernet_header->data;
             ...
             arp_packet->target_protocol_address == interface->ip_address)
    {
        ...
        // FIXME: Implement support for proxying ARP requests for other hosts, which might sometimes
        ...
        memory_copy(arp_packet->target_hardware_address, arp_packet->sender_hardware_address, 6);
        memory_copy(arp_packet->sender_hardware_address, interface->hardware_address, 6);
        ...
        message_parameter.length = (sizeof(ipv4_ethernet_header_type) + sizeof(arp_packet_type));
