...
// Process a TCP packet. For now, always terminate the connection so that the sender gets a 'Connection refused'. (Just dropping
// the packets is rather impolite IMO)
//
                        ...
                        int length __attribute__((unused)), mailbox_id_type output_mailbox_id)
{
    ...
    // FIXME: Support this. For now, we treat everything as being in the CLOSED state.
    ...
    if (old_tcp_header->acknowledge == 0)
    {
        ...
        tcp_header->acknowledgement_number = system_byte_swap_u32(system_byte_swap_u32(old_tcp_header->sequence_number) + 1);
