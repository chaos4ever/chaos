...
void udp_init(void)
{
    ...
    memory_allocate((void **) udp_hash_pointer, SOCKET_HASH_SIZE * sizeof(socket_type *));
}
...
static void udp_socket_insert(socket_type *socket)
{
    ...
    list_node_insert((list_type **) &udp_hash[hash_index], (list_type *) socket);
}
...
return_type udp_send(void *data, unsigned int length, socket_type *socket)
{
    ...
    if (!route_find(socket->destination_address, &interface, &direct, &ethernet_structure))
    {
    ...
    ipv4_ethernet_header_create(ethernet_address, interface->hardware_address, IPV4_ETHERNET_PROTOCOL_IPV4, ethernet_header);
    ...
    udp_header_create(socket->destination_port, socket->source_port, length, udp_header);
...
void udp_packet_receive(ipv4_interface_type *interface __attribute__((unused)), ipv4_ethernet_header_type *ethernet_header,
                        int length, mailbox_id_type output_mailbox_id __attribute__((unused)))
{
    ...
    if (socket != NULL)
    {
        ...
        log_print_formatted(&log_structure, LOG_URGENCY_DEBUG, "ull: %u", packet->length);

        packet_list_type **packet_list_pointer = &socket->packet_list;
        list_node_insert((list_type **) packet_list_pointer, (list_type *) packet);
    }
}
