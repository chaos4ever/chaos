...
typedef struct
{
    // Is this application listening for keyboard events? And how verbose should we be?
    bool wants_keyboard;
    int keyboard_type;
...
typedef struct
{
    ...
    // If type is VIDEO_MODE_TYPE_TEXT, characters. Otherwise, pixels.
    int width;
    int height;
    int depth;
    int type;

    // The current state of the console. Used for ANSI escape sequences.
    unsigned int state;
    ...
    // The current application receives input events. (keyboard and mouse).
    console_application_type *active_application;

    // This linked list holds information about all the applications connected to this specific console.
