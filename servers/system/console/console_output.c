...
// FIXME: Write small inline functions for all escape sequences and call them, so code won't have to be duplicated.
void console_output(console_type *console, const char *string)
{
    ...
    while (string[string_index] != '\0')
    {
        ...
        {
            // The string is, so far, nice and full of printable characters.
            case CONSOLE_STATE_CHARACTER:
            {
                switch (string[string_index])
                {
                    ...
                    case 32 ... 255:
                    {
                        // FIXME: inverse, underline etc... Support all features (except maybe blink) if in graphic mode.
                        int buffer_index;
                        unsigned int length;
                        ucs2_type ucs2;
                    ...
                    // Unsupported character - ignored for now. Should possibly be replaced with a ? mark or similar...
                    default:
                    {
                        break;
                    }
            ...
            case CONSOLE_STATE_ESCAPE:
            {
                switch (string[string_index])
                {
                    ...
                    case '0' ... '9':
                    {
                        console->numeric_argument[console->numeric_argument_index] = string[string_index] - '0';
                    ...
                    // Unknown escape command - ignored for now.
                    default:
                    {
                        break;
                    }
            ...
            case CONSOLE_STATE_NUMERIC:
            {
                switch (string[string_index])
                {
                    ...
                    case 'H':
                    {
                        ...
                        {
                            console_cursor_move(console, console->numeric_argument[0], console->numeric_argument[1]);
                        }
                    ...
                    case 'm':
                    {
                        for (unsigned int counter = 0; counter < console->numeric_argument_index + 1; counter++)
                        {
                            switch (console->numeric_argument[counter])
                            {
                                ...
                                case 30 ... 37:
                                {
                                    console_set_foreground(console, console->numeric_argument[counter] - 30);
                                    break;
                                }
                                ...
                                case 40 ... 47:
                                {
                                    console_set_background(console, console->numeric_argument[counter] - 40);
                                    break;
                                }
    ...
    // Check if the cursor position was updated. If it was, move the physical cursor.
    if (has_video &&
