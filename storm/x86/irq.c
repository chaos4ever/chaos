...
void irq_init(void)
{
    ...
    // Remap the IRQs to 20-2F. The defaults are Very Bad, since they overlap the exceptions...
    irq_remap();

    // Set up a handler for the task switcher.
    idt_setup_interrupt_gate(IDT_ENTRY(0), SELECTOR_KERNEL_CODE, dispatch_task_switcher, 0);

    idt_setup_interrupt_gate(IDT_ENTRY(1), SELECTOR_KERNEL_CODE, irq1_handler, 0);

    // IRQ 2 is used to connect the two interrupt controllers and can not be used as an ordinary IRQ.
    idt_setup_interrupt_gate(IDT_ENTRY(3), SELECTOR_KERNEL_CODE, irq3_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(4), SELECTOR_KERNEL_CODE, irq4_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(5), SELECTOR_KERNEL_CODE, irq5_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(6), SELECTOR_KERNEL_CODE, irq6_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(7), SELECTOR_KERNEL_CODE, irq7_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(8), SELECTOR_KERNEL_CODE, irq8_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(9), SELECTOR_KERNEL_CODE, irq9_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(10), SELECTOR_KERNEL_CODE, irq10_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(11), SELECTOR_KERNEL_CODE, irq11_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(12), SELECTOR_KERNEL_CODE, irq12_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(13), SELECTOR_KERNEL_CODE, irq13_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(14), SELECTOR_KERNEL_CODE, irq14_handler, 0);
    idt_setup_interrupt_gate(IDT_ENTRY(15), SELECTOR_KERNEL_CODE, irq15_handler, 0);
...
return_type irq_register(unsigned int irq_number, char *description)
{
    ...
    irq[irq_number].description = memory_global_allocate(string_length(description) + 1);
...
{
    // Make sure we're not being tricked to unregister someone else's IRQ.
    if (!irq[irq_number].allocated ||
...
// This function handles all interrupts except for the timer interrupt.
void irq_handler(unsigned int irq_number)
{
    DEBUG_MESSAGE(DEBUG, "irq %u occurred.", irq_number);
    ...
    if (!irq[irq_number].allocated)
    {
        DEBUG_MESSAGE(DEBUG, "Unexpected interrupt %u occurred!", irq_number);
    }
    else
    {
...
// Wait for the next interrupt, or if we have pending interrupts, return immediately.
return_type irq_wait(unsigned int irq_number)
{
    // If we currently are in the interrupt handler, wait until we are not.
    // FIXME: We must use a mutex here. Or something. Right now, we just hope and pray we won't get another interrupt during the
    ...
    {
        DEBUG_MESSAGE(DEBUG, "Interrupts pending %u", irq[irq_number].interrupts_pending);
        irq[irq_number].interrupts_pending--;
        return STORM_RETURN_SUCCESS;
    }
