...
// Returns the uptime in milliseconds. Its precision depends on the task switch rate, which is different on
...
return_type system_call_port_range_register(unsigned int start, unsigned int ports, char *description)
{
...
return_type system_call_memory_get_physical_address(void *virtual_address, void **physical_address)
{
    return memory_get_physical_address(virtual_address, physical_address);
}

return_type system_call_dma_register(unsigned int channel, void **dma_buffer)
{
...
return_type system_call_dma_transfer(unsigned int channel, unsigned int buffer_size, unsigned int operation,
    unsigned int transfer_mode, unsigned int autoinit)
{
    return dma_transfer(channel, buffer_size, operation, transfer_mode, autoinit);
}
...
return_type system_call_kernelfs_entry_read(kernelfs_generic_type *kernelfs_generic)
{
...
{
    return mailbox_create(mailbox_id, size, user_process_id, user_cluster_id, user_thread_id);
}
...
extern return_type system_call_service_create(const char *protocol_name,  mailbox_id_type *mailbox_id, tag_type *identification)
{
