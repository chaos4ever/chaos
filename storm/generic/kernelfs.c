// Abstract: The kernel virtual file system. In this filesystem, information about the kernel and the rest of the system is made
...
return_type kernelfs_entry_read(kernelfs_generic_type *kernelfs_generic)
{
    ...
    // FIXME: "One method to rule them all" isn't really an optimal approach. A better way would be to have an array/hash
    // with KERNELFS_CLASS_* as the key and a method pointer as the value. We don't really have an kernel hash implementation
    // at the moment, so we should consider integrating e.g. http://attractivechaos.awardspace.com/khash.h.html or writing
    // our own. https://code.google.com/p/sparsehash seems pretty good also.
    switch (kernelfs_generic->kernelfs_class)
    {
        // Get information on a thread.
        case KERNELFS_CLASS_THREAD_INFO:
        {
            kernelfs_thread_info_type *thread_info = (kernelfs_thread_info_type *) kernelfs_generic;
            ...
            while (tss_node != NULL)
            {
                if (tss_node->tss->process_id == thread_info->process_id)
                {
                    if (thread == thread_info->thread_number)
                    {
                        ...
                        // FIXME: Refactor this whole switch/case into a separate helper method.
                        // FIXME: We should have similar enum names and strings.
                        switch (tss_node->tss->state)
                        {
                            case STATE_DISPATCH:
                            {
                                string_copy_max(thread_info->state, "Running", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_MAILBOX_SEND:
                            {
                                string_copy_max(thread_info->state, "Blocked on a mailbox (sending)", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_MAILBOX_RECEIVE:
                            {
                                string_copy_max(thread_info->state, "Blocked on a mailbox (receiving)", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_MUTEX_KERNEL:
                            {
                                string_copy_max(thread_info->state, "Blocked on a kernel mutex", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_MUTEX_USER:
                            {
                                string_copy_max(thread_info->state, "Blocked on a user mutex", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_IDLE:
                            {
                                string_copy_max(thread_info->state, "Waiting for idle timeslice", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_ZOMBIE:
                            {
                                string_copy_max(thread_info->state, "Zombie (countdown to extinction)", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_INTERRUPT_WAIT:
                            {
                                string_copy_max(thread_info->state, "Waiting for hardware interrupt", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_SLEEP:
                            {
                                string_copy_max(thread_info->state, "Sleeping", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_LOG_READ:
                            {
                                string_copy_max(thread_info->state, "Reading kernel log", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_BLOCKED_PARENT:
                            {
                                string_copy_max(thread_info->state, "Blocked on parent", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            case STATE_BLOCKED_BOOT:
                            {
                                string_copy_max(thread_info->state, "Blocked during bootup", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }

                            default:
                            {
                                string_copy_max(thread_info->state, "Unknown (kernel bug)", MAX_STATE_NAME_LENGTH - 1);
                                break;
                            }
                        }
        ...
        {
            kernelfs_thread_info_verbose_type *thread_info = (kernelfs_thread_info_verbose_type *) kernelfs_generic;
            unsigned int thread = 0;
            tss_list_type *tss_node;
            ...
            while (tss_node != NULL)
            {
                if (tss_node->tss->process_id == thread_info->process_id)
                {
                    if (thread == thread_info->thread_number)
                    {
                        ...
                        thread_info->main_memory = tss_node->tss->allocated_pages * SIZE_PAGE;
                        thread_info->stack_memory = tss_node->tss->stack_pages * SIZE_PAGE;
                        thread_info->instruction_pointer = tss_node->tss->instruction_pointer;

                        // Now, reset the timeslices counter in the TSS, so we can see which threads are currently getting most of
                        // the time.
        ...
        {
            kernelfs_process_info_type *process_info = (kernelfs_process_info_type *) kernelfs_generic;
            ...
            {
                DEBUG_MESSAGE(DEBUG, "process_info->process_number > number_of_processes");
                break;
            }
            ...
            else
            {
                ...
                string_copy_max(process_info->name, node->name, MAX_PROCESS_NAME_LENGTH);
            }
            break;
        }
        ...
        {
            kernelfs_self_type *kernelfs_self = (kernelfs_self_type *) kernelfs_generic;
        ...
        {
            kernelfs_memory_info_type *kernelfs_memory_info = (kernelfs_memory_info_type *) kernelfs_generic;

            kernelfs_memory_info->total_memory = memory_physical_get_number_of_pages() * SIZE_PAGE;
        ...
        {
            // FIXME: We need to have an array/list of CPU structures. Also, we must decide how this should be done in an
            // architecture independant manner. This can wait until we start porting storm...
            kernelfs_cpu_info_type *kernelfs_cpu_info = (kernelfs_cpu_info_type *) kernelfs_generic;
        ...
        {
            kernelfs_storm_info_type *kernelfs_storm_info = (kernelfs_storm_info_type *) kernelfs_generic;
        ...
        {
            kernelfs_video_info_type *video_info = (kernelfs_video_info_type *) kernelfs_generic;
            ...
            video_info->frame_buffer_address = (void *) multiboot_info.frame_buffer_address;

            debug_print("%x %x\n", video_info->frame_buffer_address, BASE_PROCESS_PAGETABLES);
            break;
        }
#endif
        ...
        {
            kernelfs_port_info_type *port_info = (kernelfs_port_info_type *) kernelfs_generic;
            port_range_type *port = port_list;
        ...
        case KERNELFS_CLASS_IRQ_AMOUNT:
        {
            ...
            *irqs = 0;
            ...
            for (unsigned int index = 0; index < IRQ_LEVELS; index++)
            {
        ...
        {
            kernelfs_irq_info_type *kernelfs_irq_info = (kernelfs_irq_info_type *) kernelfs_generic;
        ...
        // Try to read a log entry from the kernel log. If there is nothing to be read and the block flag is set, this will block
        // the reader.
        case KERNELFS_CLASS_LOG_READ:
        {
            kernelfs_log_type *kernelfs_log = (kernelfs_log_type *) kernelfs_generic;
            ...
            string_copy_max(kernelfs_log->string, log_list->string, kernelfs_log->max_string_length);
        ...
        {
            kernelfs_time_type *kernelfs_time = (kernelfs_time_type *) kernelfs_generic;
