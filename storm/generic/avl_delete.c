...
// FIXME: There are lots of nasties in this code, since it hasn't been completely converted from 'math' to C yet (the 'math'
// version had tons of goto's and stuff).
...
static void single_rotation_with_balanced_child(avl_node_type *node, avl_node_type *child);
static void single_rotation_with_unbalanced_child(avl_node_type *node, avl_node_type *child);
...
return_type avl_key_delete(avl_header_type *header, unsigned int key)
{
    // Fixme: Mutex this!
    ...
    // If the more node is NULL, we can move the less node one step up, and we're done.
    if (node->more == NULL)
    {
        if (parent == NULL)
        {
    ...
    for (; depth > 0; depth--)
    {
        ...
        else
        {
            ...
            if (child->balance == 0)
            {
                ...
                // In this case, the tree will be fully balanced, so we return.
                return;
            }
            else if (child->balance == -a[depth])
            {
                ...
                // In this case, the tree will not be balanced, so we continue up the tree.
            ...
            else
            {
                ...
                // In this case, the tree will not be balance, so we continue up the tree.
                continue;
            }
        }
    }
...
static void update_largest_child(int key, avl_node_type *node, avl_node_type *value)
{
    if (key == +1)
    {
        node->more = value;
