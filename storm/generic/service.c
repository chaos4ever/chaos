...
// responsibility to the servers to not pollute the namespace, but we trust people not to misuse this.
//
// The services are stored as a linked list in a tree. The tree is sorted on protocol name. The linked list is sorted on inverse
// chronology.
...
// Gets a pointer to the given protocol, if it exists, or NULL otherwise.
...
return_type service_create(const char *protocol_name, mailbox_id_type *mailbox_id, tag_type *identification)
{
    ...
    if (protocol == NULL)
    {
        ...
        protocol = (protocol_type *) memory_global_allocate(sizeof (protocol_type));
        protocol->name = (char *) memory_global_allocate(sizeof (char) * string_length(protocol_name) + 1);
    ...
    if (mailbox_create_kernel(&service->mailbox_id, SERVICE_MAILBOX_SIZE, PROCESS_ID_NONE, CLUSTER_ID_NONE, THREAD_ID_NONE) !=
...
// Gets all the services for the given protocol.
return_type service_get(const char *protocol_name, service_parameter_type *service_parameter, tag_type *identification_mask)
{
    ...
    {
        DEBUG_MESSAGE(DEBUG, "STORM_RETURN_INVALID_ARGUMENT (%x, %x, %x)", protocol_name, service_parameter, identification_mask);
        mutex_kernel_signal(&tss_tree_mutex);
    ...
    if (protocol == NULL)
    {
        DEBUG_MESSAGE(DEBUG, "STORM_RETURN_PROTOCOL_UNAVAILABLE (%s)", protocol_name);
        mutex_kernel_signal(&tss_tree_mutex);
...
// This is a recursive function that is called from service_protocol_get.
// FIXME: Find a better name for this.
static void protocol_get_all(service_protocol_type *protocol_info, unsigned int *index, protocol_type *node, unsigned int max)
{
    if (node == NULL)
    {
        return;
    ...
    string_copy_max(protocol_info[*index].name, node->name, MAX_PROTOCOL_NAME_LENGTH);
    protocol_info[*index].number_of_services = node->number_of_services;
    (*index)++;
}

return_type service_protocol_get(unsigned int *maximum_protocols, service_protocol_type *protocol_info)
{
    unsigned int index = 0;
