...
// The kernel-level debugger uses a Swedish translation map. If you want to change it, you'll have to hack those tables.
...
// Shows a list of the available commands, with a short description
...
void command_sanity(int number_of_arguments UNUSED, char **argument UNUSED)
{
    bool okay = TRUE;
...
// FIXME: This function isn't really generic. Move somewhere else...
...
static inline void put_character(int x, int y, int character, int character_attribute)
{
...
static int print_simple(const char *string)
{
    ...
    for (index = 0; string[index] != '\0'; index++)
    {
        ...
        if (x_position == dataarea.x_size)
        {
            ...
            // If we're passing the last line on screen, scroll everything upwards one line.
...
void debug_print_simple(const char *string)
{
    ...
    // If we're trying to write the NULL string, screw ourselves, hippies.
    if (string == NULL)
    {
        print_simple("(null)");
...
// Convert a number to an hexadecimal string notation. Padded to eight digits with zereos.
...
// Print a formatted string to screen. Only used for debugging. This is NOT a POSIX compliant function.
void debug_print(const char *format_string, ...)
{
    ...
    while (format_string[index] != 0)
    {
        ...
        {
            // Percent sign means we should get a formatting code as the next character.
            ...
            case '%':
            {
                ...
                switch (format_string[index])
                {
                    ...
                    case 'c':
                    {
                        ...
                        put_character(x_position, y_position, character, debug_text_attribute);
                    ...
                    // Hexadecimal number. Padded to eight characters with zeroes.
                    case 'p':
                    case 'x':
                    {
                        char string[9];
                    ...
                    default:
                    {
                        put_character(x_position, y_position, format_string[index], debug_text_attribute);
...
void debug_crash_screen(const char *message, volatile storm_tss_type *dump_tss)
{
    ...
    debug_print("  Thread was dispatched %u times.\n", dump_tss->timeslices);
...
void debug_crash_screen(const char *message, volatile storm_tss_type *dump_tss)
{
    ...
    debug_print("  Physical memory: %u KByte used. %u KByte free.\n", memory_physical_get_used() * 4, memory_physical_get_free() * 4);
    debug_print("  Global memory: %u KByte used. %u KByte free.\n", memory_global_get_used() / 1024, memory_global_get_free() / 1024);
...
static int debug_line_parse(char *source, debug_arguments_type *destination)
{
                ...
                switch (character_code)
                {
                    ...
                    case DEBUG_SCAN_CODE_BACK_SPACE:
                    {
                        if (where > 0)
                        {
                            ...
                            x_position--;
                            put_character(x_position, y_position, ' ', debug_text_attribute);
                            place_cursor(x_position, y_position);
                        }
                        break;
                    }
...
void debug_print_simple(char *string UNUSED)
{
}
