...
bool port_range_link(unsigned int start, unsigned int ports, char *description)
{
    port_range_type *node = port_list;
    port_range_type *new_node;

    // Check if the port range is already allocated. We have two cases when we have to deny the request. This is when
    // the requested range is inside another portrange, or another portrange starts or ends inside the requested
    // region. */
    ...
    while (node != NULL)
    {
        // The first case.
        if (start >= node->start &&
            (start + ports) <= (node->start + node->length))
        {
            return FALSE;
        }
    ...
    // OK, the requested region is free, we can open up the ports in the thread's I/O-map. But first, let's mark this
    // region as used.
    new_node = (port_range_type *) memory_global_allocate(sizeof(port_range_type));
...
void port_init(void)
{
    // Allocate some reserved regions. FIXME: maybe a struct holding all the regions and a loop?
...
return_type port_range_register(unsigned int start, unsigned int ports, char *description)
{
    unsigned int port;
    int index;
    ...
    // First of all, we have to check if the process has access to any ports at all.
    // FIXME: Should be a capability check instead.
    ...
    if (current_tss->iomap_size < BIT_IN_BYTES(start + ports - 1))
    {
        ...
        current_tss = memory_global_allocate(sizeof(storm_tss_type) + BIT_IN_BYTES(start + ports));

        DEBUG_MESSAGE(DEBUG, "Allocating %u bytes.", (sizeof(storm_tss_type) + BIT_IN_BYTES(start + ports - 1)));
    ...
    // Add the port range to the caller's I/O-map.
...
return_type port_range_unregister(unsigned int start)
{
    ...
    // Did we run out of the list? If so, this thread has not previously allocated a port range starting at this base,
    // so just abort silently.
    if (port == NULL)
    {
        return STORM_RETURN_INVALID_ARGUMENT;
    }
    ...
    if (port->previous == NULL && port->next == NULL)
    {
        // This is the only entry; everything gets very simple.
        port_list = NULL;
    }
    else if (port->previous == NULL)
    {
        // It doesn't have a "previous" pointer, so we can assume that this is the first entry. If so, make the next
        // entry the start of the list.
