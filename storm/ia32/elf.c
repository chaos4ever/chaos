...
return_type elf_execute(u8 *image, char *parameter_string, process_id_type *process_id)
{
    ...
    // Well, that was the identification block. Now for the really important part of the ELF header...
    //
    // Start by finding the size of the eventual BSS section. The first section is used to mark undefined section references, so
    ...
    for (index = 1; index < elf_header->section_header_entries; index++)
    {
        ...
        if (section_header->flags == (ELF_SECTION_FLAG_WRITE | ELF_SECTION_FLAG_ALLOCATE) &&
    ...
    // FIXME: Make a real ELF implementation some day. This *will* not work in some quite common cases. Also try to modularize
    // this part of the kernel and have it removed, once kernel init is done.
    for (index = 1; index < elf_header->section_header_entries; index++)
    {
        section_header = (section_header_type *)
                         (image + elf_header->section_header_offset +
        ...
        if ((section_header->address < BASE_PROCESS_SPACE_START ||
             section_header->address + section_header->size >= BASE_PROCESS_SPACE_END) &&
